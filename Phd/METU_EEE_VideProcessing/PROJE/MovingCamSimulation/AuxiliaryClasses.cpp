...
C3DMotion::C3DMotion(CString s)
{
	int sepIndex = s.Find(";");
	CString param = s.Left(sepIndex);
	m_Vx= atof(param);
	int sepIndex2 = s.Find(";", ++sepIndex);
	param = s.Mid(sepIndex, sepIndex2-sepIndex);
	m_Vy = atof(param);
	sepIndex = s.Find(";", ++sepIndex2);
	param = s.Mid(sepIndex2, sepIndex-sepIndex2);
	m_Vz = atof(param);
