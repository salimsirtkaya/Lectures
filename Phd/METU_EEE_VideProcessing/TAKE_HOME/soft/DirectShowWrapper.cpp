...
//
HRESULT CDirectShowWrapper::GrabCurrentFrame(unsigned char * & image,long& w,long& h)
{
	if (!m_bSetUp)
		return CO_E_NOTINITIALIZED;
	...
	long bitmapSize;
	CHECK_FAILED_DSW(m_pVideoGrab->GetCurrentBuffer(&bitmapSize, (long *)NULL));
	image = (unsigned char *) malloc(bitmapSize * sizeof(char));
	if (!image)
