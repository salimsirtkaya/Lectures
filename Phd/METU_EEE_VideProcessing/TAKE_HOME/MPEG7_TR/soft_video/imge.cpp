...
void CImge::Feature_inv(CImge &I_x, CImge &I_y,int wind,int block_size)
{
	CImge temp=*this;
	...
	/*Randomly select a point in the current block that have an intensity gradient 
	  greater than a prescribed threshold*/

	for (int b_no=0; b_no<block_no;b_no++)
	{	

		/*check whether the randomly selected point is inside the gradient calculation area*/
		if (x<=wind)
			x = x + wind + 1;
		if (x>=(h-wind))
			x = x - wind - 1;
		...
		{
			mFeature[b_no].x = (double)x;
			mFeature[b_no].y = (double)y;					
		}
	}
}
