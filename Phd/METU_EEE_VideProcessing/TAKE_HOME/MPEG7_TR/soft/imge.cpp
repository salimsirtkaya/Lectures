...
/*	for (int i=wind;i<h-wind;i++)
	{
		for (int j=wind;j<w-wind;j++)
		{
			Grad.SetAllPxlsTo(0);

			for (win_x=i-wind;win_x<=i+wind;win_x++)
			{
				for(win_y=j-wind;win_y<=j+wind;win_y++)
				{
	...
	Eig.MinMax(min_eig,max_eig,indx,indy);*/
...
/*	for (int x=10;x<h-10;x++)
	{
		for (int y=10;y<w-10;y++)
		{		
			if(Eig(x,y)>=0.7*max_eig)
			{
				mFeature[Feature_no].x = x;
				mFeature[Feature_no].y = y;
				mFeature[Feature_no].vx = 0;
				mFeature[Feature_no].vy = 0;
...
#define Kernel(I,x,y)  0.1406 *I(2*(x),2*(y)) + 0.0938 *(I(2*(x)-1,2*(y))+I(2*(x),2*(y)-1)+I(2*(x)+1,2*(y))+I(2*(x),2*(y)+1))+\
...
CImge CImge::PyramidSeed()
{
	...
	for(int ind1=0;ind1<res.height;ind1++)
		for(int ind2=0;ind2<res.width;ind2++)
			res(ind1,ind2)=Kernel(temp,(ind1+1),(ind2+1));
	return res;
}
...
void CImge::Buff2ColorIm(unsigned char* buf, short h, short w, BUFFER_FROM buffer_from, char ch)
{
	int colorCode = MapCharToColorCode(ch);
	Resize(h,w);
	int i,j;
	int b_index;
if (buffer_from==IMAGE)
	{
		...
		for (i=height-1;i>=0;i--)
		{
			b_index=i*(width*3+padding);
			for (j=width-1;j>=0;j--)
				Image[i][j]=buf[j*3+b_index+colorCode];
		}
	}
	...
	else
	{
		...
		for (i=height-1;i>=0;i--)
		{
			...
			{
				b_index=(h_index-j)*3;
				Image[i][width-1-j]=buf[b_index-colorCode];
			}
		}
...
int CImge::MapCharToColorCode(char ch)
{
	int chCode;
	switch (ch)
	{
...
void CImge::GetColorFromFromFile(CString filename, char ch)
{
	int chCode= MapCharToColorCode(ch);
	CGdiplusWrapper source(filename);
	...
	BYTE* buf;
	buf=source.GetPixels(bmpdata);
	Buff2ColorIm(buf,height,width,IMAGE,ch);
	delete [] buf;
}
