...
CRenderingArea::CRenderingArea()
{
	...
	m_currentObjectPen.CreatePen(PS_SOLID, 1, 0x0080ff);
	m_previousObjectPen.CreatePen(PS_DOT, 1, 0x00ff80);
	m_axisPen.CreatePen(PS_DOT, 1, 0xffff00);
}
...
void CRenderingArea::Grid(CDC &dc, int xSpace, int ySpace, bool gridOn)
{
	if (gridOn)
	{
		...
		dc.SetTextColor(0xffff00);
		dc.TextOut(-8,-3,"o");

/*		dc.MoveTo(-xOrg+5, yOrg-5);
		dc.Ellipse(-xOrg
*/		dc.TextOut(-20, -yOrg, "Y-Axis");
		dc.TextOut(xOrg-50, -10, "X-Axis");

		CFont font, *oldFont;
		font.CreatePointFont(70,"Arial");
		oldFont = dc.SelectObject(&font);
		dc.TextOut(-10, -60, "50");
		dc.TextOut(-15, -160, "150");
		dc.TextOut(50, 0, "50");
		dc.TextOut(150, 0, "150");
...
void CRenderingArea::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	m_manager->WriteMouseLocInfo( p.x, p.y);
