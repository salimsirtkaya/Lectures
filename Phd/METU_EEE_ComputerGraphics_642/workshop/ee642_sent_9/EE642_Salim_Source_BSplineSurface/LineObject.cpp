...
{
	int i,j,s,tmaxu,tmaxw,n,m,kindex, k;
...
//	int i, j;
	...
	else
	{
		...
		for (iu=0;iu<meshNo;iu++)
		{
			...
			{
				w=tmaxw*float(iw)/float(meshNo-0.9999);
				u=tmaxu*float(iu)/float(meshNo-0.9999);
				
				for (i=0;i<n+1;i++)
				{
					N = calculateN(i,u,ku,knotu);
					for (j=0;j<m+1;j++)
					{
						M = calculateN(j,w,kw,knotw);
						tempx=M*N*BSx[i][j]+tempx;
						tempy=M*N*BSy[i][j]+tempy;
						tempz=M*N*BSz[i][j]+tempz;
					}
				}
