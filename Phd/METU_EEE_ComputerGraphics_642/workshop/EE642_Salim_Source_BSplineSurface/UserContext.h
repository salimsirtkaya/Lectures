...
{
private:
	int surfaceMethod; // 3:LinearCoons, 4:BicubicCoons, 5:Bezier, 6:B-Spline
	...
	void LinearCoons();
	void BezierSurface();
	void ReadLinearCoonsFile(CString file);
	void ReadBicubicCoonsFile(CString file);
	void ReadBezierFile(CString file);
	void ReadBSplineFile(CString file);
	double tempMm[4][4][3];
	CBezierSurface bezierSurface;
	CBSplineSurface bSplineSurface;
public:
	~CSurfaceContext();
	static CUserContext* GetInstance();
	void PaintRenderArea(CDC &dc){};
