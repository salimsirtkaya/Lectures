...
{
	m_bClosedObject = true;
}


CCompGraphObject::CCompGraphObject(bool closed)
{
	m_bClosedObject = closed;
	//m_points.
}
...
CCompGraphObject* CCompGraphObject::GetNewInstance()
{
	...
	else
	{
		...
		{
			newObj = new CCompGraphObject(this->m_bClosedObject);
		}
	}
	return newObj;
}
