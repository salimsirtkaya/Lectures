...
void CBSplineSurface::DrawGraphObject(CDC &dc, bool closed)
{
	...
	int knotu[20],knotw[20];
	int iu,iw;
	double temp1,temp2,tempx,tempy,tempz;
	FILE *read_stream, *in;
	float u,w;
...
/*		double sumx,sumy,sumz;
		...
		//////////////////////////////////////////////////////////////////////////////
		dc.SelectObject(pOld);
		for(i=0,u=0;i<=(15);i++,u+=du)
		{
			sumx=sumy=sumz=0;
			for(k=0,w=0;k<=15;k++,w+=dw)
			{
				for(int t=0;t<=4;t++)
				{
					for(int j=0;j<3;j++)
					{
						sumx=calculateN(t,u,order,knotu)*calculateN(j,w,order,knotw)*BSx[t][j]+sumx;
						sumy=calculateN(t,u,order,knotu)*calculateN(j,w,order,knotw)*BSy[t][j]+sumy;
						sumz=calculateN(t,u,order,knotu)*calculateN(j,w,order,knotw)*BSz[t][j]+sumz;
					}
				}
				if(w==0)
				{
		...
		//--
		//--
		for(i=0,w=0;i<=(15);i++,w+=dw)
		{
			sumx=sumy=sumz=0;
			for(k=0,u=0;k<=15;k++,u+=du)
			{
				for(int t=0;t<=4;t++)
				{
					for(int j=0;j<3;j++)
...
*/		float N,M;
		int meshNo = 15;
		for (iu=0;iu<meshNo;iu++)
		{
			for (iw=0;iw<meshNo;iw++)
			{
				w=tmaxw*float(iw)/meshNo;
				u=tmaxu*float(iu)/meshNo;
				
				for (i=0;i<n;i++)
				{
					N = calculateN(i,u,order,knotu);
					for (j=0;j<m;j++)
					{
						M = calculateN(j,w,order,knotw);
						tempx=N*M*BSx[i][j]+tempx;
						tempy=N*M*BSy[i][j]+tempy;
						tempz=N*M*BSz[i][j]+tempz;
					}
				}
...
float CBSplineSurface::calculateN(int i,double t,int k, int xknot[])
{
	double w,w1,q,a;
	float res;
	if(k==1)
	{
		if((xknot[i]<=t)&&(t<xknot[i+1])) 
			res=1;
		else 
			res=0;
	}
	else
	{
		...
		if(w==0) w=99999.0;
		if(w1==0) w1=99999.0;
		q=float((t-xknot[i])*calculateN(i,t,k-1,xknot))/float(w);
		a=float((xknot[i+k]-t)*calculateN(i+1,t,k-1,xknot))/float(w1);
		res=(q+a);
	}
	return res;
}
