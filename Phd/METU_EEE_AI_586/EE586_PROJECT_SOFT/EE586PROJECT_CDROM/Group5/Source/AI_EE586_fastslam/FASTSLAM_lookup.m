function FASTSLAM_lookup(port,baud,time)
close all;
...
%***************************************
ARENA=zeros(800,800);
ARENA(300,400)=-270; %start position
ARENA(340:360,450:470)=1;
ARENA(550:570,300:380)=1;
ARENA(340:400,130:150)=1;
...
[initialPose, poseUpdate, INPUT_kiks, tMax] = Kinematic_lookup(0);
...
while(t<tMax)
    ...
    Measurement(t+1,:)  = convertMeasurements_lookup(mean_sensor_data);
    
    
    %%%%fastSlam algorithm
    
    if (Measurement(t+1,1) > 25)  % if there is valuable measurement
        for m=1:no_of_particles
            ...
            for n=0:PARTICLE_SET(t).particles(m).no_of_landmarks-1
                ...
                mCap(2) = atan2(deltaY,deltaX)-PARTICLE_SET(t+1).particles(m).pose(3);
                if mCap(2)<0
                    mCap(2) = mCap(2)+2*pi;
                end
                %%Calculate Jacobian
