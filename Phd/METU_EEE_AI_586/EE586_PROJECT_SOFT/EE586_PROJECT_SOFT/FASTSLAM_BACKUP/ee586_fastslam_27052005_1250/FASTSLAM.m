...
while(t<7)
    ...
    if (Measurement(t,1) > 0.2)  % if there is valuable measurement
        for m=1:no_of_particles
            ...
            for n=0:PARTICLE_SET(t).particles(m).no_of_landmarks-1
                ...
                w(n+1)=exp(-0.5*(Measurement(t,:)-mCap)*inv(Q)*(Measurement(t,:)-mCap)')/ sqrt(det(Q));
            end;
            %set importance factor for new landmark ss 0.5 may not be true
            w(PARTICLE_SET(t).particles(m).no_of_landmarks+1) = 0;
            ...
            for n=1:PARTICLE_SET(t+1).particles(m).no_of_landmarks            
                if n==PARTICLE_SET(t).particles(m).no_of_landmarks+1  %is new feature
                    ...
                    %rearranged
                    PARTICLE_SET(t+1).particles(m).landmark(n).mean(1) = PARTICLE_SET(t+1).particles(m).pose(1) + Measurement(t,1) * cos(Measurement(t,2)*pi/180);
                    PARTICLE_SET(t+1).particles(m).landmark(n).mean(2) = PARTICLE_SET(t+1).particles(m).pose(2) + Measurement(t,1) * sin(Measurement(t,2)*pi/180);
