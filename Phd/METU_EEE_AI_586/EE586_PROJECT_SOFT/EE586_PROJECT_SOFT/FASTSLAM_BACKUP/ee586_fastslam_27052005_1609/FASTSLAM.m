...
for i=1:no_of_particles
    ...
    PARTICLE_SET(1).particles(i).pose(3) = 90; % orientation
...
Measurement(2,:) = [0 0];
Measurement(3,:) = [0 0];
Measurement(4,:) = [5*sqrt(2)+0.2*randn 45+randn];
Measurement(5,:) = [5+0.2*randn randn];
Measurement(6,:) = [5*sqrt(2)+0.2*randn -45+randn];
Measurement(7,:) = [0 0];
Measurement(8,:) = [0 0];
Measurement(9,:) = [0 0];
Measurement(10,:) = [8*sqrt(2)+0.2*randn 135+randn];
Measurement(11,:) = [8+0.2*randn 180+randn];
Measurement(12,:) = [8*sqrt(2)+0.2*randn 225+randn];
    ...
    if (Measurement(t+1,1) > 0.2)  % if there is valuable measurement
        for m=1:no_of_particles
            ...
            for n=0:PARTICLE_SET(t).particles(m).no_of_landmarks-1
                ...
                mCap(2) = atan2(deltaY,deltaX)*180/pi;
                %%Calculate Jacobian
                ...
                weight(n+1)=exp(-0.5*(Measurement(t+1,:)-mCap)*inv(Q)*(Measurement(t+1,:)-mCap)')/ sqrt(det(Q));
            end;
            %set importance factor for new landmark ss 0.5 may not be true
            weight(PARTICLE_SET(t).particles(m).no_of_landmarks+1) = 0.0001;
            % calculate the new number of features
            [ttemp,nCap] = max(weight);
            ...
            for n=1:PARTICLE_SET(t+1).particles(m).no_of_landmarks            
                if n==PARTICLE_SET(t).particles(m).no_of_landmarks+1  %is new feature
                    ...
                    %rearranged
                    PARTICLE_SET(t+1).particles(m).landmark(n).mean(1) = PARTICLE_SET(t+1).particles(m).pose(1) + Measurement(t+1,1) * cos(Measurement(t+1,2)*pi/180);
                    PARTICLE_SET(t+1).particles(m).landmark(n).mean(2) = PARTICLE_SET(t+1).particles(m).pose(2) + Measurement(t+1,1) * sin(Measurement(t+1,2)*pi/180);
