function [t, node] = Astar_misplaced();
...
if ~isempty(a1),
    ...
    if (a~=1) & (b~=1), %the a1 state is encountered for the first time
        ...
        st(i).state=a1;
        st(i).pred=pop.state;
        st(i).h=misp_dist_f(st(i).state,goal);
    ...
    elseif a==1 & (st(kk).f > (misp_dist_f(a1,goal)+pop.g+1)), %a1 is already in closed and f of a1 is less.
        st(kk).g=pop.g+1;
        st(kk).f=(st(kk).h+st(kk).g);
        st(kk).pred=pop.state;
    elseif ((b==1) & (opened(kb).f > (misp_dist_f(a1,goal)+pop.g+1))), %a1 is already in opened and f of a1 is less.
...
if ~isempty(a2),
    ...
    elseif a==1 & (st(kk).f > (misp_dist_f(a2,goal)+pop.g+1)), %a2 is already in closed and f of a2 is less.
        st(kk).g=pop.g+1;
        st(kk).f=(st(kk).h+st(kk).g);
        st(kk).pred=pop.state;
        
    elseif ((b==1) & (opened(kb).f > (misp_dist_f(a2,goal)+pop.g+1))), %a2 is already in opened and f of a2 is less.,
        opened(kb)=[]; %remove previous from opened.
        r=r-1;
...
if ~isempty(a3),
    ...
    elseif a==1 & (st(kk).f > (misp_dist_f(a3,goal)+pop.g+1)), %a3 is already in closed and f of a3 is less.
        st(kk).g=pop.g+1;
        st(kk).f=(st(kk).h+st(kk).g);
        st(kk).pred=pop.state;
    elseif ((b==1) & (opened(kb).f > (misp_dist_f(a3,goal)+pop.g+1))), %a3 is already in opened and f of a3 is less.,
        opened(kb)=[]; %remove previous from opened.
        r=r-1;
...
if ~isempty(a4),
    ...
    elseif a==1 & (st(kk).f > (misp_dist_f(a4,goal)+pop.g+1)), %a4 is already in closed and f of a4 is less.
        st(kk).g=pop.g+1;
        st(kk).f=(st(kk).h+st(kk).g);
        st(kk).pred=pop.state;
        
    elseif ((b==1) & (opened(kb).f > (misp_dist_f(a4,goal)+pop.g+1))), %a4 is already in opened and f of a4 is less.
        opened(kb)=[]; %remove previous from opened.
        r=r-1;
